...
- `@eyueldk/aisdk-toolkit-browser` types align with **`puppeteer`**; **`puppeteer` is a peer dependency** (install alongside the package).
...
- Private monorepo root package is `aimachine`; published workspace packages include `@eyueldk/aisdk-toolkit-browser` in `packages/browser`, `@eyueldk/aisdk-toolkit-todos` in `packages/todos`, and `@eyueldk/aisdk-toolkit-filesystem` in `packages/filesystem`.
...
- `@eyueldk/aisdk-toolkit-browser` `Session.close()` is async, stops inspectors, and closes the page; use `await session.close()` in teardown.
- Root `pnpm test` runs `@eyueldk/aisdk-toolkit-browser` then `@eyueldk/aisdk-toolkit-todos` then `@eyueldk/aisdk-toolkit-filesystem` in sequence so parallel Vitest plus Puppeteer does not time out.
...
- `@eyueldk/aisdk-toolkit-todos` uses camelCase AI SDK tool names (e.g. `writeTodos`, `viewTodos`); todo status literals are `pending`, `in_progress`, and `completed`.
